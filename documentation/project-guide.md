...
9. The package viewer's "download all" fires one programmatic download per document (spaced apart, not zipped); the browser may still prompt to allow multiple downloads, and a recipient who dismisses that prompt only gets the first file.
