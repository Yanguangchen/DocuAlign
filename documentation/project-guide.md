...
| Saved-report dashboard | Lists, filters by period (Today / month), deletes, and shares report metadata and documents | Implemented |
...
| `src/lib/reports.js` | Firestore report CRUD, timestamp normalization, period bounds, inclusive date filtering |
...
**The filter is a period, not a hand-typed range.** Two controls, mutually exclusive: a **Today** button (a toggle — a second click clears it) and a **month** selector (`<input type="month">`). Each resolves through `dayRange()` or `monthRange()` to a pair of inclusive date-only bounds, which is what `filterReportsByDate` already reads as midnight through 23:59:59.999. `monthRange` finds a month's last day by asking for day 0 of the following month, so 28/29/30/31 need no lookup table; both helpers format in **local time**, because `toISOString()` would put "today" on the wrong calendar day for anyone away from UTC. `filterReportsByDate` itself still accepts finer `YYYY-MM-DDTHH:mm` bounds and leaves an unparseable bound open, so the filtering core stays general even though the UI now only produces whole days and months.

The payoff is that a period is **complete by construction**: unlike a From/To pair, neither control can be left half-specified, so the packaging checkbox below never has to guard against an open-ended bound. Delete uses a two-click arm/confirm interaction and permanently deletes only the report document; it does not discover or revoke existing public shares.
...
**Packaging a whole period.** One checkbox above the grid selects every report the filter is currently showing, each at card level so it contributes its whole document set. It is gated on a period being chosen at all — with none, the selection would be every report the dashboard has ever loaded — and until one is, the checkbox is disabled with the reason on screen. Its label names the active period (*Package every document saved today* / *…saved in June 2026*) rather than saying "this period", so it still reads correctly once the filter has been scrolled past. While ticked it is a live statement rather than a one-off action: switching period re-selects against the period now on screen. Unticking any single report drops it back to unchecked, and a period matching nothing clears the selection entirely — `render` blanks the grid on an empty result so `syncBundleSelection` prunes what those cards were holding, since a selection surviving behind a hidden grid would still be counted and still be published.
