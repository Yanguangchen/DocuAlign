...
| Saved-report dashboard | Lists, filters to one day, deletes, and shares report metadata and documents | Implemented |
...
| `src/lib/reports.js` | Firestore report CRUD, timestamp normalization, day bounds, inclusive date filtering |
...
The dashboard fetches every `docuAlignReports` document ordered by `createdAt` descending, normalizes timestamps, and filters dates in browser memory.

**The filter is one day, not a hand-typed range.** A single calendar (`<input type="date">`) picks it, and a **Today** button is a shortcut that fills that same input — not a second mode. The input is therefore the only thing holding the answer, so the two controls cannot disagree; Today shows as pressed exactly while the picked day *is* today, which means reaching today through the calendar lights it up as well, and a second click on the button clears the input again.

`dayRange()` turns the picked value into a pair of inclusive bounds — both ends the day itself, which `filterReportsByDate` already reads as midnight through 23:59:59.999, so a whole day needs no special casing in the filter. It rejects a well-shaped but impossible date (`2026-02-31`, `2026-13-01`) by round-tripping the parse: the `Date` constructor rolls those forward rather than failing, so without the check 31 February would silently filter to 3 March. `todayValue()` formats in **local time**, because `toISOString()` would put "today" on the wrong calendar day for anyone away from UTC.

`filterReportsByDate` itself still accepts finer `YYYY-MM-DDTHH:mm` bounds and leaves an unparseable bound open, so the filtering core stays general even though the UI now only produces whole days.

The payoff is that the period is **complete by construction**: unlike a From/To pair it cannot be left half-specified, so the packaging checkbox below never has to guard against an open-ended bound. Delete uses a two-click arm/confirm interaction and permanently deletes only the report document; it does not discover or revoke existing public shares.
...
**Packaging a whole day.** One checkbox above the grid selects every report the filter is currently showing, each at card level so it contributes its whole document set. It is gated on a day being picked at all — with none, the selection would be every report the dashboard has ever loaded — and until one is, the checkbox is disabled with the reason on screen. Its label names the day (*Package every document saved today* / *…saved on 15 June 2026*, worded through the shared `periodPhrase` helper the empty-state message also uses) rather than saying "this date", so it still reads correctly once the filter has been scrolled past. Dates are named with `Intl` in the **viewer's** locale, so the tests derive expected wording the same way rather than assuming an ordering. While ticked it is a live statement rather than a one-off action: picking another date re-selects against the day now on screen. Unticking any single report drops it back to unchecked, and a day matching nothing clears the selection entirely — `render` blanks the grid on an empty result so `syncBundleSelection` prunes what those cards were holding, since a selection surviving behind a hidden grid would still be counted and still be published.
...
9. Multi-package parent ZIP names are capped at a practical filename length. When every complete package folder name will not fit, the parent keeps the first package name where space permits and counts the remainder.
