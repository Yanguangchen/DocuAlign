...
4. **Packages (Grouped Share Links):**
   ...
   * Each packaged document is published as its **own** share token so it stays individually revocable. Bundles store TOKENS only (1..`MAX_BUNDLE_REPORTS` = 100); never embed report snapshots (see §1.3 — measured to exceed the 1000-expression rules limit). Keep the cap in `src/lib/share.js` and `firestore.rules` in step.
   ...
   * `view.html`'s package panel lists every document as its own card, in `exportOrder`'s Summary-first / sampling-date order, plus two buttons: **Download all N documents (ZIP)** (`downloadEveryDocument` in `src/view-report.js`) packs every document into one ZIP through `docuAlignZip.createArchive`, each still its own PDF inside it — one download, no merge — and **Print all N documents** (`buildPrintJob`) merges every document through `src/pdf-merge.js` and hands the result to a hidden iframe's `print()`.
