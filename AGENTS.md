...
7. **A missing appendix photograph must never leave the reference page's own in place.** Signatures may (they are identical on every report RAK issues); photographs may not, because they are that cargo hold's evidence. `rak-report-pdf.js` marks appendix images `evidence: true` and clears the box to "Photograph unavailable" when there are no bytes. Never "restore" the old skip-the-whiteout behaviour for them. **`pageFivePlan` plans BOTH boxes from `positions`, never from `photos`** — a report that maps no photographs at all would otherwise plan nothing, and a box that is not planned is never cleared. A real client workbook whose photographs had not been pasted in shipped page 5 carrying the reference's own JIAHE 99 sample under a HONG HAI 9 job ref; `src/rak-report-pdf.test.js` covers the empty-list case as well as the no-bytes one.
...
10. **The signatures are told from the photographs by the shape they sit in, never by counting.** The sign-off sets its two signatures SIDE BY SIDE — the same row give or take a row of drift, at opposite ends of the page — while the appendix STACKS its two photographs in one column. `signaturePair` in `src/report-mapping.js` uses exactly that (`SIGNATURE_ROW_SPAN`, `SIGNATURE_COLUMN_SPAN`), and only what is not a signature can be a photograph. The rule it replaced took the last two pictures on the sheet as the photographs and the two above them as the signatures, which holds only while a report HAS photographs: a client workbook whose photographs were not yet pasted in had the sign-off's own two signatures as its last two pictures, and printed them blown up under "Photographs of sample received" — while leaving the sign-off with none of its own. Never reintroduce positional counting here.

11. **A value that is not of its own kind must be reported.** Reading a workbook at the wrong row does not fail — it produces a complete, correctly laid out, signed report carrying its neighbour's values. `describeAnomalies` in `src/report-mapping.js` checks the fields whose form is unmistakable (job reference, the three dates, the key measurements) and `reportValueShapes` in `src/workspace.js` logs `ImplausibleReportValues` naming the group and field. It deliberately does not know which row is right — only that a sampling date reading `HH9-638N` is a voyage number. Keep it silent on a correctly mapped workbook: a check that warns on every export is not read on the one that matters, so only add a field here whose shape is genuinely unambiguous. **`N/A` is a result, not a gap** — the lab writes it for a test it did not run (7 of the 9 reports in the two known workbooks do), and flagging it would make the metals table warn on almost every export.

12. **A result has no shape, so check the block around it — `BLOCK_CANARIES`.** A shear stress of `45.2` is indistinguishable from a moisture content of `45.2`; no per-value rule can ever reach one. What can be checked is that the block it sits in is where the reader thinks it is, using the values the FORM fixes: the seven sieve sizes, the twelve element names and their JTC limits, the silt requirement, the shear condition and limit, the appendix headings. The sieve and element columns are read and never drawn — they exist in the model only as by-products — so two comparisons cover 64 result values that nothing else reaches. Each was identical across all nine reports of the two known workbooks, which drifted independently; that is evidence, not proof, so these stay **warnings**. RAK may legitimately revise a sieve set or a JTC limit, and when they do the warning is the prompt to update the expected value, never a reason to delete the canary. `describeGradingOrder` adds the one physical law available: cumulative passing cannot rise as the sieve gets finer, so a column that climbs was read off the wrong rows even when every value in it is a plausible percentage.

13. **Where two sources carry the same fact, compare them.** A shape check asks whether a value is well-formed; these ask whether it agrees with something that must agree with it, which is the only way to catch a value that is wrong and well-formed at the same time. Three exist, all silent on both known workbooks: `describeDateOrder` (a sample is taken, then received, then reported on — the check that catches a shifted cover block whose values are all still valid dates, which is exactly what shape checks cannot see); `describeJobRefAgreement` (`CV1` and `TR1!AE2` both carry the job reference and are read independently, so `buildReport` keeps both readings in `jobRefSources` — 48 characters in the share payload — and reports a disagreement, since each reading alone is a perfectly good job reference and only the pair is wrong); and `describeShearAgreement` (`TR1`'s summary row is its `SB1` curve's peak to the nearest whole kPa, exactly, in all nine known reports — **nothing else in the mapper reads `SB1` at all**, so a block that drifted on either sheet shows up here and nowhere else). Curves pair to summary rows by their own normal stress, never by position. Do not "simplify" any of these into a single-value check: the second source IS the check.
...
3. **REQUIREMENT — One `CV1` + `TR1` Sheet Group = One Test Report:**
   ...
   > [!CAUTION]
   > **The cover is read by its labels, never by the sample's rows.** Every `CV1` field is a label in column C, a colon, and the value in column K on the SAME row. Those rows are `SampleInput.xlsx`'s, not the form's: a client workbook listing two more test standards pushes the identity block down, and fixed addresses then read the row above each label — the vessel name printed as the sample id, the sampling date as the date received, on a signed report with every other value correct and nothing to show the shift. `coverReader` in `src/report-mapping.js` resolves each field to its own label's row (matched on letters and digits only, so `VOY No.` and `VOY No` are the same label), keeping the sample's row only as the fallback for a sheet that lacks the label, and resolving a repeated label to the occurrence nearest that row. The address's second line and the two six-row lists keep the reference page's fixed shape — only where they start moves. Never reintroduce a bare `text(coverSheet, "K29")`; this is the same lesson as §2a, on the other half of the page.
